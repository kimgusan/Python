...
while True:
    ...
    # 사용자가 1,2 번을 선택하지 않고 3번을 선택하였을 때
        ...
        if d_name in name_list:
            # 해당 name_list에 인덱스 번호 확인 및 index 변수 대입
            index = name_list.index(d_name)
    ...
    # 사용자가 1,2,3 번을 선택하지 않고 4번을 선택하였을 때
        ...
        if menu_choice == 1:
            ...
            if search_name in name_list:
                ...
                print(f"{name_list} 안에 {search_name}는 존재 합니다.")
                # 오류 메세지를 출력하지 않기 위해 continue 사용
                continue
        ...
        # 사용자가 입력한 값의 오차범위 내에 있는 값을 넣은 빈 리스트 생성
            ...
            for i in range(len(price_list)):
                ...
                if search_price * 0.5 <= price_list[i] <= search_price * 1.5:
                    # 빈 인덱스에 price_list 요소 값 추가
                    count_index.append(price_list[i])
