...
# 현재 위치에 따라 변경되는 경로
# ./ => 현재 경로, ../ => 이전경로, ./src/images, .././a/b, src/images
...
# with를 사용하면, 자동으로 file이 close 된다 // class의 매직메소드 __enter__, __exit__, __del__ 생각해보기
...
with open('food.txt','r', encoding='utf-8') as file:
    ...
    while line != '':
        ...
        if line == '햄버거\n':
            # content를 치킨으로 변경해서 담기
            content += '치킨\n'
            continue

        # 수정 대상이 아닌 줄은 그대로 content 담기
        content += line
