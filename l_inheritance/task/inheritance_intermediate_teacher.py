...
# 정확히 어떤 것을 검사할 지는 사용자아게 전달받은 key로 확인할 수 있다.
def check(*, key: str, value: str) -> dict:
    # 저장소(DB)에서 각 은행 정보를 가져온 뒤
    for bank in Bank.banks:
        # 각 은행에서 회원의 정보를 가져온다.
        for user in bank:
            # 전달받은 키워드(key)로 회원의 정보가 value와 같은지 검사한다
            if user[key] == value:
...
class Bank:
    ...
    # 어떤 은행에서 개설하는 지 bank_choice로 전달받는다.
    # 개설에 필요한 모든 회원 정보는 **kwargs로 받는다.
    @classmethod
    def open_account(cls, bank_choice, **kwargs):
        ...
        # 은행 DB에 고객의 정보를 담을 때, dic 타입으로 변환해서 받는다
        # check 함수에서 원하는 key로 호원의 정보를 찾기 위해서!
        cls.banks[bank_choice - 1].append(user.__dict__) # ( bank 라는 객체를 딕셔너리로 바꿔준다 !!!)
        # 개설돈 회원 정보를 리턴한다.
    ...
    def check_account_number(account_number: str) -> dict:
        # check 함수에서 검사할 key 와 value를 전달한다.
    ...
    while True:
        ...
        while True:
            ...
            if menu_choice == 1:
                ...
                # 재정의한 __str__()이 사용되고, 호원의 전체 정보를 확인한다.
                print(user)
            ...
            # 출금
                ...
                if user is not None:
                    if user['password'] == input(password_message):
                        ...
                        # 출금 수수료가 50% 이기 때문에, 잔액 검사 시 1.5를 곱해준다.
                    ...
                    if user['password'] == input(password_message):
                        while True:
                            ...
                            if Bank.check_account_number(account_number) is None:
                                break
                        # 새롭게 설정한 계좌번호로 등록한다.
                        # 계좌를 개설할 때 __dict__ 로 저장했다.
                        # 이때 __dict__를 수정하는 것 보다, 객체로 직접 접근해서 바꾸는 것이 안전하다
                        # 결국, __dict__ 도 self를 받아서 만들어진 객체이므로, 뿌리인 self로 접근하는 것이 좋다
